				...
				UINT8 data_id, UINT8 offset, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	*/
	...
	/* clear read ready bit before trigger dsi read */
	if (dsi_i == 0) {
		DSI_OUTREGBIT(cmdq, struct DSI_INT_STATUS_REG,
		      DSI_REG[0]->DSI_INTSTA, RD_RDY, 0x00000000);
	}
	...
	if (MIPITX_IsEnabled(module, cmdq)) {
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
			...
			if (i == 0) { /* only enable DSI0's INT */
				...
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG, DSI_REG[i]->DSI_INTEN, RD_RDY, 0);
