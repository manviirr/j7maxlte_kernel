...
#define get_cluster_min_cpufreq(id)		\
	...
	: 0)
#define	get_cluster_hmp_target_cpufreq(id)	\
	((PPM_CLUSTER_L == id)	\
	? ppm_main_info.cluster_info[id].dvfs_tbl[DVFS_HMP_L_OPP_NUM_1P469GHZ].frequency	\
	: ppm_main_info.cluster_info[id].dvfs_tbl[DVFS_HMP_LL_OPP_NUM_1P508GHZ].frequency)
...
/* smart detect boost (L+) */
extern void ppm_set_smart_detect_boost(unsigned int smart_detect);
/* sched HMP booster */
extern int ppm_hica_root_cluster_changed(int cluster);
