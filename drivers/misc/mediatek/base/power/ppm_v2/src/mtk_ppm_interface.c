...
int ppm_hica_root_cluster_changed(int cluster)
{
	int ret = -1;
...
#else
		if(ppm_hica_root_cluster_changed(cluster) == -1)
			ppm_err("changing root cluster to %d fail!!!", cluster);
#endif
	} else
		ppm_err("echo (cluster_id or -1 to cancel) > /proc/ppm/root_cluster\n");
...
void ppm_set_smart_detect_boost(unsigned int smart_detect)
{
	ppm_lock(&ppm_main_info.lock);
					...
					size_t count, loff_t *pos)
{
	...
	if (!kstrtoint(buf, 10, &smart_detect)) {
		ppm_set_smart_detect_boost(smart_detect);
	} else
