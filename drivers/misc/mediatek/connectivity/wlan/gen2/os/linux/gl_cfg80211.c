...
#define LINKSPEED_MAX_RANGE_11BGN 3000
	...
	if (prGlueInfo->eParamMediaStateIndicated != PARAM_MEDIA_STATE_CONNECTED) {
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			...
			DBGLOG(REQ, WARN, "last link speed, status=%d, used cache rate=%d\n"
				, rStatus, sinfo->txrate.legacy);
			return -EBUSY;
		} else if (u4Rate == 0) {
			sinfo->txrate.legacy = prGlueInfo->u4LinkSpeedCache;
			DBGLOG(REQ, WARN, "last link speed, current rate=%d, used cache rate=%d\n"
				, u4Rate, sinfo->txrate.legacy);
		} else {
	...
	if (prGlueInfo->eParamMediaStateIndicated != PARAM_MEDIA_STATE_CONNECTED) {
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			...
			DBGLOG(REQ, WARN, "status :%d ,used cache rssi :%d\n", rStatus, sinfo->signal);
			return -EBUSY;
		} else if ((i4Rssi == PARAM_WHQL_RSSI_MIN_DBM) || (i4Rssi == PARAM_WHQL_RSSI_MAX_DBM)) {
			sinfo->signal = prGlueInfo->i4RssiCache;
			DBGLOG(REQ, WARN, "current rssi:%d ,used cache rssi:%d\n", i4Rssi, sinfo->signal);
		} else {
