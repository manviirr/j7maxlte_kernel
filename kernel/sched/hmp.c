...
static unsigned long topapp_mask = 0xFF;
...
static int task_is_fg(struct task_struct *p)
{
	...
	/*If this task is top-app*/
	if (cpumask_equal(tsk_cpus_allowed(p), to_cpumask(&topapp_mask))) {
		return 2;
	}
...
#ifdef CONFIG_SCHED_HMP_BOOST
#define task_waked(f) ((SD_BALANCE_WAKE == f)?1:0)
#endif
				     /*
		...
		int prev_cpu, int new_cpu)
{
	...
	if ((task_created(sd_flag) && !task_low_priority(p->prio)) ||
			(hmp_boost && task_waked(sd_flag) && task_is_fg(p))) {
#else
	if (task_created(sd_flag) && !task_low_priority(p->prio)) {
#endif
