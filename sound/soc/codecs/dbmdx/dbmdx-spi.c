...
#include "dbmdx-spi.h"
#ifdef CONFIG_MTK_SPI /* MTK set SPI config */
#include "../../../drivers/spi/mediatek/mt6757/mtk_spi.h"
#endif
...
#ifdef CONFIG_MTK_SPI /* MTK set SPI config */
int spi_common_set_param(struct spi_device *spidev)
{
...
#ifdef CONFIG_MTK_SPI /* MTK set SPI config */
	ret = spi_common_set_param(p->client);
	if (ret) {
		dev_err(p->dev, "%s: spi_common_set_param failed\n", __func__);
		ret = -EINVAL;
		goto out_err_kfree;
	}	
#endif
