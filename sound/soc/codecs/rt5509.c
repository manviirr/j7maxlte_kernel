...
#define SPK_MAX_EXCURSION ((30 << 14) / 100)
#define ASPK	(240)
#define TMAX_THRESHOLD (90)
#define TMAX_OVERCOUNT (20)
static inline int rt5509_update_data_logging(struct rt5509_chip *chip)
{
	struct rt5509_calib_classdev *pcalib_dev = &chip->calib_dev;
	struct snd_soc_codec *codec = chip->codec;
	uint32_t xlmt = 0, scale = 0, excur = 0;
	int32_t dcr = 0, dcr_25 = 0, t_raw = 0;
	uint32_t xpeak_raw = 0;
	uint32_t tmax_overcount = 0, tmax_count = 0, xmax_count = 0;
	int ret = 0;

	ret = snd_soc_read(codec, RT5509_REG_CALIB_BL);
	if (ret < 0)
		return ret;
	xlmt = ret & 0xffff;
	scale = (xlmt << 10) / SPK_MAX_EXCURSION;
	ret = snd_soc_read(codec, RT5509_REG_CALIB_DCR);
	dcr_25 = ret & 0xffffff;
	/* read dcr */
	...
	if (dcr >= (1 << 20))
		dcr = (1 << 20) - 1;
	t_raw = (dcr - (dcr_25 >> 4)) * (ASPK + 25);
	/* read excursion */
	ret = snd_soc_write(codec, RT5509_REG_SPKRPTSEL, 0x0F);
	if (ret < 0)
		return ret;
	ret = snd_soc_read(codec, RT5509_REG_SPKRPT);
	excur = ret & 0xffffff;
	xpeak_raw = (excur << 10) / scale;
	if (xpeak_raw >= (SPK_MAX_EXCURSION << 5))
		xpeak_raw = (SPK_MAX_EXCURSION << 5);
	mutex_lock(&pcalib_dev->var_lock);
	tmax_overcount = pcalib_dev->tmax_overcount;
	tmax_count = pcalib_dev->tmax_count;
	xmax_count = pcalib_dev->xmax_count;
	if (xpeak_raw > pcalib_dev->xmax_raw)
		pcalib_dev->xmax_raw = xpeak_raw;
	if (xpeak_raw > (SPK_MAX_EXCURSION << 5))
		xmax_count++;
	if (t_raw > pcalib_dev->tmax_raw)
		pcalib_dev->tmax_raw = t_raw;
	if (t_raw > (int32_t)((TMAX_THRESHOLD - 25) << 19))
		tmax_overcount++;
	else
		tmax_overcount = 0;
	if (tmax_overcount >= TMAX_OVERCOUNT) {
		tmax_overcount = 0;
		tmax_count++;
	}
	/* unit: .1'c */
	pcalib_dev->t = 250 + ((t_raw * 10) >> 19);
	/* unit: .1'c */
	pcalib_dev->tmax = 250 + ((pcalib_dev->tmax_raw * 10) >> 19);
	/* uint: .001 mm */
	pcalib_dev->xpeak = ((xpeak_raw * 1000) >> 19);
	/* uint: .001 mm */
	pcalib_dev->xmax = ((pcalib_dev->xmax_raw * 1000) >> 19);
	pcalib_dev->tmax_overcount = tmax_overcount;
	pcalib_dev->tmax_count = tmax_count;
	pcalib_dev->xmax_count = xmax_count;
	mutex_unlock(&pcalib_dev->var_lock);
	return 0;
}

static inline int rt5509_polling_apply_ampon(struct rt5509_chip *chip)
{
	uint32_t dcr_25, dcr_min, dcr_max, dcr_cur;
	int ret = 0;

	dev_dbg(chip->dev, "%s\n", __func__);
	if (!chip->wk_backup) {
		ret = snd_soc_read(chip->codec, RT5509_REG_VOLUME);
		if (ret < 0)
			goto out_apply_ampon;
		chip->wk_vol = ret;
		ret = snd_soc_read(chip->codec, RT5509_REG_INITIMPLDMU);
		if (ret < 0)
			goto out_apply_ampon;
		chip->wk_step1 = ret;
		ret = snd_soc_read(chip->codec, RT5509_REG_IMPLDMU);
		if (ret < 0)
			goto out_apply_ampon;
		chip->wk_step2 = ret;
		chip->wk_backup = 1;
	}
	dcr_25 = snd_soc_read(chip->codec, RT5509_REG_CALIB_DCR);
	dcr_25 &= 0xffffff;
	dcr_25 >>= 4;
	/* dcr_min = dcr / 2, dcr_max = (rspk * 3) / 2 */
	dcr_min = dcr_25 >> 1;
	dcr_max = (dcr_25 * 3) >> 1;
	ret = snd_soc_write(chip->codec, RT5509_REG_SPKRPTSEL, 0x0d);
	if (ret < 0)
		goto out_apply_ampon;
	ret = snd_soc_read(chip->codec, RT5509_REG_SPKRPT);
	dcr_cur = ret & 0xffffff;
	dev_dbg(chip->dev, "dcr_cur -> 0x%08x\n", dcr_cur);
	if (dcr_cur >= dcr_max || dcr_cur <= dcr_min) {
		dev_warn(chip->dev, "over min or max range\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_VOLUME,
				    chip->wk_vol + 12);
		if (ret < 0)
			dev_err(chip->dev, "apply vol min6 fail\n");
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_FUNCEN,
					  0x20, 0x00);
		if (ret < 0)
			dev_err(chip->dev, "apply limiter disable fail\n");
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_TDEN,
					  0x20, 0x00);
		if (ret < 0)
			dev_err(chip->dev, "apply ATC disable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_INITIMPLDMU, 0x00);
		if (ret < 0)
			dev_err(chip->dev, "apply step1 disable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_IMPLDMU, 0x00);
		if (ret < 0)
			dev_err(chip->dev, "apply step2 disable fail\n");
	} else {
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_FUNCEN,
					  0x20, 0x20);
		if (ret < 0)
			dev_err(chip->dev, "apply limiter enable fail\n");
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_TDEN,
					  0x20, 0x20);
		if (ret < 0)
			dev_err(chip->dev, "apply ATC enable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_VOLUME,
				    chip->wk_vol);
		if (ret < 0)
			dev_err(chip->dev, "apply vol fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_INITIMPLDMU,
				    chip->wk_step1);
		if (ret < 0)
			dev_err(chip->dev, "apply step1 enable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_IMPLDMU,
				    chip->wk_step2);
		if (ret < 0)
			dev_err(chip->dev, "apply step2 enable fail\n");
	}
	ret = rt5509_update_data_logging(chip);
	if (ret < 0)
		dev_err(chip->dev, "update data logging fail\n");
out_apply_ampon:
...
static inline int rt5509_polling_apply_ampoff(struct rt5509_chip *chip)
{
	int ret = 0;
	...
	if (chip->wk_backup) {
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_FUNCEN,
					  0x20, 0x20);
		if (ret < 0)
			dev_err(chip->dev, "restore limiter enable fail\n");
		ret = snd_soc_update_bits(chip->codec, RT5509_REG_TDEN,
					  0x20, 0x20);
		if (ret < 0)
			dev_err(chip->dev, "restore ATC enable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_VOLUME,
				    chip->wk_vol);
		if (ret < 0)
			dev_err(chip->dev, "restore vol fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_INITIMPLDMU,
				    chip->wk_step1);
		if (ret < 0)
			dev_err(chip->dev, "restore step1 enable fail\n");
		ret = snd_soc_write(chip->codec, RT5509_REG_IMPLDMU,
				    chip->wk_step2);
		if (ret < 0)
			dev_err(chip->dev, "restore step2 enable fail\n");
		chip->wk_backup = 0;
	}
	return 0;
}

static void rt5509_polling_work(struct work_struct *work)
{
	struct rt5509_chip *chip = container_of(work, struct rt5509_chip,
						polling_work.work);
	int ret = 0;

	if (chip->amp_on)
		ret = rt5509_polling_apply_ampon(chip);
	else
		ret = rt5509_polling_apply_ampoff(chip);

	if (ret < 0)
		dev_err(chip->dev, "apply polling fail\n");
}
...
static int rt5509_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (ret < 0)
		goto err_out_probe;
	INIT_DELAYED_WORK(&chip->polling_work, rt5509_polling_work);
	device_init_wakeup(codec->dev, true);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		chip->amp_on = 1;
		if (chip->calibrated) {
			pm_stay_awake(codec->dev);
			/* 100ms wait for stable */
			schedule_delayed_work(&chip->polling_work,
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (chip->calibrated) {
			...
			rt5509_polling_apply_ampoff(chip);
			pm_relax(codec->dev);
		}
