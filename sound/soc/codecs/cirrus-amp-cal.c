/*
 * cirrus-amp-calibration.c -- CS35L35 calibration driver
 *
...
#define L_STPILOTPARAMSIN_IENABLE 0x0A038A
#define L_STADAPTPARAMSIN_FXPEAKTHRCONST 0x0A0316
#define L_STOUTPUTPARAMS_FXRE 0x0A0416

#define L_STTSPARAMSIN_FXTREF 0x0A02C0
#define L_STTSPARAMSIN_FXREREFINV 0x0A02C2

#define R_STPILOTPARAMSIN_IENABLE 0x0A05A8
#define R_STADAPTPARAMSIN_FXPEAKTHRCONST 0x0A0534
#define R_STOUTPUTPARAMS_FXRE 0x0A0634

#define R_STTSPARAMSIN_FXTREF 0x0A04DE
#define R_STTSPARAMSIN_FXREREFINV 0x0A04E0
...
struct cirrus_amp_calib_ration {
	...
	unsigned int fx_peak_L;
	unsigned int fx_peak_R;
	int inv_dc_resistance_L;
	int inv_dc_resistance_R;
	int temperature;
...
static int cirrus_left_amp_calib_read_irdc(int *value);
static int cirrus_right_amp_calib_read_irdc(int *value);
...
int cirrus_amp_calib_apply(void)
{
	...
	ret = cirrus_left_amp_calib_read_irdc(&irdc_L);
	if (ret < 0)
		return ret;

	ret = cirrus_right_amp_calib_read_irdc(&irdc_R);
	if (ret < 0)
		return ret;
	...
	dev_info(calib->dev,
		 "Writing calibration: 1/rDC_L: %d, 1/rDC_R: %d, Temp: %d\n",
		 irdc_L, irdc_R, temp);

	ret = regmap_write(calib->regmap, L_STTSPARAMSIN_FXREREFINV, irdc_L);
	if (ret)
		return ret;

	ret = regmap_write(calib->regmap, L_STTSPARAMSIN_FXTREF, temp);
	if (ret)
		return ret;

	ret = regmap_write(calib->regmap, R_STTSPARAMSIN_FXREREFINV, irdc_R);
	if (ret)
		return ret;

	ret = regmap_write(calib->regmap, R_STTSPARAMSIN_FXTREF, temp);
	if (ret)
		return ret;
...
static int cirrus_amp_calib_start(void)
{
	...
	ret = regmap_read(calib->regmap, L_STADAPTPARAMSIN_FXPEAKTHRCONST,
			&(calib->fx_peak_L));
	if (ret)
		return ret;

	ret = regmap_read(calib->regmap, L_STPILOTPARAMSIN_IENABLE, &val);
	if (ret)
		return ret;

	/* Set Left Amp fx_peak to 0 */
	ret = regmap_write(calib->regmap, L_STADAPTPARAMSIN_FXPEAKTHRCONST, 0x0);
	if (ret)
		return ret;

	if (val != 0x3) {
		dev_warn(calib->dev,
			"Left Amp Pilot Tone not enabled by preconfig: %d\n", val);
		/* Enable pilot tone */
		ret = regmap_write(calib->regmap, L_STPILOTPARAMSIN_IENABLE, 0x3);
		if (ret)
			return ret;
	}

	ret = regmap_read(calib->regmap, R_STADAPTPARAMSIN_FXPEAKTHRCONST,
			&(calib->fx_peak_R));
	if (ret)
		return ret;

	ret = regmap_read(calib->regmap, R_STPILOTPARAMSIN_IENABLE, &val);
	if (ret)
		return ret;

	/* Set Right Amp fx_peak to 0 */
	ret = regmap_write(calib->regmap, R_STADAPTPARAMSIN_FXPEAKTHRCONST, 0x0);
	if (ret)
		return ret;

	if (val != 0x3) {
		dev_warn(calib->dev,
			"Right Amp Pilot Tone not enabled by preconfig: %d\n", val);
		/* Enable pilot tone */
		ret = regmap_write(calib->regmap, R_STPILOTPARAMSIN_IENABLE, 0x3);
		if (ret)
			return ret;
	}
...
static int cirrus_amp_calib_end(void)
{
	...
	/* restore fx_peak */
	ret = regmap_write(calib->regmap, L_STADAPTPARAMSIN_FXPEAKTHRCONST,
			calib->fx_peak_L);
	if (ret)
		return ret;

	ret = regmap_write(calib->regmap, R_STADAPTPARAMSIN_FXPEAKTHRCONST,
			calib->fx_peak_R);
	if (ret)
		return ret;
...
static int cirrus_amp_calib_store(void)
{
	...
	dev_info(calib->dev,
		 "Writing calibration: 1/rDC_L: %d, 1/rDC_R: %d, Temp: %d\n",
		 calib->inv_dc_resistance_L, calib->inv_dc_resistance_R,
		 calib->temperature);
	...
	ret = cirrus_amp_calib_write_file(FILEPATH_CRUS_RDC_L_CAL,
				      calib->inv_dc_resistance_L);
	if (ret < 0)
		return ret;

	ret = cirrus_amp_calib_write_file(FILEPATH_CRUS_RDC_R_CAL,
				      calib->inv_dc_resistance_R);
	if (ret < 0)
		return ret;
...
static void cirrus_amp_calib_work(struct work_struct *work)
{
	...
	ret = regmap_read(calib->regmap, L_STOUTPUTPARAMS_FXRE, &raw_rept);
	if (ret) {
		dev_err(calib->dev, "Failed to read Left Amp rePT: %d\n", ret);
		goto error;
	}
	...
	if ((_rept) < calib->imp_min ||
	    (_rept) > calib->imp_max) {
		dev_err(calib->dev,
			"Left Amp->rept: 0x%08x or redc: 0x%08x out of range %d<x<%d\n",
	...
	calib->inv_dc_resistance_L = rept;

	ret = regmap_read(calib->regmap, R_STOUTPUTPARAMS_FXRE, &raw_rept);
	if (ret) {
		dev_err(calib->dev, "Failed to read Right Amp rePT: %d\n", ret);
		goto error;
	}
	...
	if ((_rept) < calib->imp_min ||
	    (_rept) > calib->imp_max) {
		dev_err(calib->dev,
			"Right Amp->rept: 0x%08x or redc: 0x%08x out of range %d<x<%d\n",
			_rept, _rept, calib->imp_min, calib->imp_max);
		goto error;
	}
	...
	calib->inv_dc_resistance_R = rept;
...
static int cirrus_amp_calib_read_file(char *filename, int *value)
{
	...
	ret = __vfs_read(calib_filp, str, sizeof(str), &calib_filp->f_pos);
	if (ret != sizeof(str)) {
...
static int cirrus_amp_calib_write_file(char *filename, int value)
{
	...
	ret = __vfs_write(calib_filp, str, sizeof(str), &calib_filp->f_pos);
	if (ret != sizeof(str)) {
...
static int cirrus_left_amp_calib_read_irdc(int *value)
{
	if (calib->inv_dc_resistance_L == INT_MIN) {
		int ret = cirrus_amp_calib_read_file(FILEPATH_CRUS_RDC_L_CAL,
						 &calib->inv_dc_resistance_L);
		if (ret < 0)
			return ret;
	}

	*value = calib->inv_dc_resistance_L;
	return 0;
}

static int cirrus_right_amp_calib_read_irdc(int *value)
{
	if (calib->inv_dc_resistance_R == INT_MIN) {
		int ret = cirrus_amp_calib_read_file(FILEPATH_CRUS_RDC_R_CAL,
						 &calib->inv_dc_resistance_R);
		if (ret < 0)
			return ret;
	}

	*value = calib->inv_dc_resistance_R;
	return 0;
}
...
static ssize_t cirrus_left_amp_calib_irdc_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	...
	ret = cirrus_left_amp_calib_read_irdc(&value);
	if (!ret)
...
static ssize_t cirrus_left_amp_calib_irdc_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t size)
{
	int ret, dc;
	...
	calib->inv_dc_resistance_L = ret;

	ret = cirrus_amp_calib_write_file(FILEPATH_CRUS_RDC_L_CAL, ret);
	if (ret < 0) {
		calib->running = ret;
		mutex_unlock(&calib->lock);
		return ret;
...
static ssize_t cirrus_right_amp_calib_irdc_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	int ret, value = 0;
	...
	ret = cirrus_right_amp_calib_read_irdc(&value);
	if (!ret)
		ret = sprintf(buf, "%d", value);
	else
		calib->running = ret;
...
static ssize_t cirrus_right_amp_calib_irdc_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t size)
{
	int ret, dc;
	...
	calib->inv_dc_resistance_R = ret;

	ret = cirrus_amp_calib_write_file(FILEPATH_CRUS_RDC_R_CAL, ret);
	if (ret < 0) {
		calib->running = ret;
		mutex_unlock(&calib->lock);
		return ret;
...
static DEVICE_ATTR(irdc_left, S_IRUGO | S_IWUSR | S_IWGRP,
		   cirrus_left_amp_calib_irdc_show, cirrus_left_amp_calib_irdc_store);
static DEVICE_ATTR(irdc_right, S_IRUGO | S_IWUSR | S_IWGRP,
		   cirrus_right_amp_calib_irdc_show, cirrus_right_amp_calib_irdc_store);
...
static struct attribute *cirrus_amp_calib_attr[] = {
	...
	&dev_attr_irdc_left.attr,
	&dev_attr_irdc_right.attr,
	&dev_attr_temp.attr,
...
static int __init cirrus_amp_calib_init(void)
{
	...
	calib->inv_dc_resistance_L = INT_MIN;
	calib->inv_dc_resistance_R = INT_MIN;
...
MODULE_DESCRIPTION("CS35L35 Calibration driver");
