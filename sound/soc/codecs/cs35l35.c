		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		dev_info(codec->dev, "In %s Event: PRE_PMU\n", __func__);
	...
	case SND_SOC_DAPM_POST_PMD:
		dev_info(codec->dev, "In %s Event: POST_PMD\n", __func__);
		regmap_update_bits(cs35l35->regmap, CS35L35_PWRCTL1,
					CS35L35_DISCHG_FILT_MASK,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		dev_info(codec->dev, "In %s Event: POST_PMU\n", __func__);
				 ...
				 struct snd_soc_dai *dai)
{
	...
	dev_info(codec->dev, "In %s CLKRate: %d\n", __func__, srate);
				...
				int clk_id, unsigned int freq, int dir)
{
	...
	dev_info(codec->dev, "In %s freq: %d\n", __func__, freq);
...
static irqreturn_t cs35l35_irq(int irq, void *data)
{
	...
	dev_info(cs35l35->dev, "\t0x%x 0x%x 0x%x 0x%x\n", sticky1, sticky2,
				sticky3, sticky4);
