...
#ifdef CONFIG_SND_SOC_CS47L15
	[GPIO_AUD_I2S0] = {"aud_pins_i2s0", false, NULL},
	[GPIO_AUD_I2S1] = {"aud_pins_i2s1", false, NULL},
	[GPIO_AUD_I2S2] = {"aud_pins_i2s2", false, NULL},
	[GPIO_AUD_I2S3] = {"aud_pins_i2s3", false, NULL},
#endif
};
#endif
...
void AudDrv_GPIO_probe(void *dev)
{
	...
	if (extbuck_fan53526_exist) {	/* is e2 */
		...
		pr_warn("%s(), e2 PCB, update gpio name, high = %s, low = %s\n",
			__func__,
			aud_gpios[GPIO_HPDEPOP_HIGH].name, aud_gpios[GPIO_HPDEPOP_LOW].name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(aud_gpios); i++) {
		...
		if (IS_ERR(aud_gpios[i].gpioctrl)) {
			...
			pr_err("%s: [%s] is not supported. (%d)\n",
				__func__, aud_gpios[i].name, ret);
		} else {
			aud_gpios[i].gpio_prepare = true;
			pr_info("%s: [%s] init is done.\n",
...
#ifdef CONFIG_SND_SOC_CS47L15
int AudDrv_GPIO_I2S0(int bEnable)
{
	int retval = 0;

	retval = AudDrv_GPIO_Select(GPIO_AUD_I2S0);

	return retval;
}

int AudDrv_GPIO_I2S1(int bEnable)
{
	int retval = 0;

	retval = AudDrv_GPIO_Select(GPIO_AUD_I2S1);

	return retval;
}

int AudDrv_GPIO_I2S2(int bEnable)
{
	int retval = 0;

	retval = AudDrv_GPIO_Select(GPIO_AUD_I2S2);

	return retval;
}

int AudDrv_GPIO_I2S3(int bEnable)
{
	int retval = 0;

	retval = AudDrv_GPIO_Select(GPIO_AUD_I2S3);
...
static int __init dt_get_extbuck_info(unsigned long node, const char *uname, int depth, void *data)
{
	...
	tags =
	    (struct devinfo_extbuck_tag *)of_get_flat_dt_prop(node, "atag,extbuck_fan53526", &size);
