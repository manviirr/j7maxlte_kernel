...
#ifdef MTK_AUDIO_EXT_MCLK_SUPPORT
void Enable_Sync_I2S0_I2S3_With_MCLK(uint32 SampleRate,
				uint32 wLenBit, bool low_jitter_on)
{
	/*
	 * wLenBit : 1:Soc_Aud_I2S_WLEN_WLEN_32BITS
	 * 0:Soc_Aud_I2S_WLEN_WLEN_16BITS
	 */
	uint32 Audio_I2S0 = 0;
	uint32 Audio_I2S3 = 0;
	...
	Audio_I2S0 |= low_jitter_on << 12; /* Low jitter mode */
	...
	SetCLkMclk(Soc_Aud_I2S3, SampleRate);
	...
	/* Enable MCLK */
	EnableI2SCLKDiv(Soc_Aud_I2S0_MCKDIV, true); /* Enable I2S0 MCLK */
	EnableI2SCLKDiv(Soc_Aud_I2S3_MCKDIV, true); /* Enable I2S0 MCLK */
}

void Disable_Sync_I2S0_I2S3_With_MCLK(void)
{
	/* Disable I2S0/3 */
	Afe_Set_Reg(AFE_I2S_CON3, 0x0, 0x1);
	...
	/* Disable MCLK */
	EnableI2SCLKDiv(Soc_Aud_I2S0_MCKDIV, false);
	EnableI2SCLKDiv(Soc_Aud_I2S3_MCKDIV, false);
}

void Enable_Sync_I2S1_I2S2(uint32 SampleRate, uint32 wLenBit, bool low_jitter_on)
{	
	/*wLenBit : 1:Soc_Aud_I2S_WLEN_WLEN_32BITS /0:Soc_Aud_I2S_WLEN_WLEN_16BITS */
	uint32 Audio_I2S1 = 0;
	uint32 Audio_I2S2 = 0;

	Afe_Set_Reg(AFE_ADDA_TOP_CON0, 1, 0x1); /* Using External ADC, Not PMIC ADC */

	/*Afe_Set_Reg(AUDIO_TOP_CON1, 0x4,  0x4);*/  /* I2S_SOFT_Reset  4 wire i2s mode*/
	Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 5,  0x1 << 5); /* I2S0 clock-gated */
	Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 6,  0x1 << 6); /* I2S3 clock-gated */

	/* Set I2S2 configuration */
	Audio_I2S2 |= 0x0 << 31; /* No SWAP L/R Channel */
	Audio_I2S2 |= 0x0 << 23; /* No Invert L/R BCLK */
	Audio_I2S2 |= 0x0 << 22; /* Debug Usage */
	Audio_I2S2 |= 0x0 << 21; /* Debug Usage */
	Audio_I2S2 |= 0x0 << 20; /* No mloopback */
	Audio_I2S2 |= 0x8 << 24; /* Update Timing */
	Audio_I2S2 |= low_jitter_on << 12; /* Low jitter mode */
	Audio_I2S2 |= SampleRateTransform(SampleRate, Soc_Aud_Digital_Block_I2S_IN_2) << 8;
	Audio_I2S2 |= (Soc_Aud_I2S_FORMAT_I2S << 3);
	Audio_I2S2 |= (wLenBit << 1);
	Afe_Set_Reg(AFE_I2S_CON2, Audio_I2S2, MASK_ALL);
	pr_debug("Audio_I2S2= 0x%x\n", Audio_I2S2);

	SetSampleRate(Soc_Aud_Digital_Block_MEM_I2S, SampleRate); /* set I2S0 sample rate */

	/* Set I2S1 configuration */
	Audio_I2S1 |= low_jitter_on << 12; /* Low jitter mode */
	Audio_I2S1 |= SampleRateTransform(SampleRate, Soc_Aud_Digital_Block_I2S_IN_2) << 8;
	Audio_I2S1 |= Soc_Aud_I2S_FORMAT_I2S << 3; /*  I2s format */
	Audio_I2S1 |= wLenBit << 1; /* WLEN */
	Afe_Set_Reg(AFE_I2S_CON1, Audio_I2S1, AFE_MASK_ALL);
	pr_debug("Audio_I2S1= 0x%x\n", Audio_I2S1);
	...
	/*Afe_Set_Reg(AUDIO_TOP_CON1, 0,  0x4);*/  /* Clear I2S_SOFT_Reset  4 wire i2s mode*/
...
void Disable_Sync_I2S1_I2S2(void)
{
	/* Disable I2S1/2 */
