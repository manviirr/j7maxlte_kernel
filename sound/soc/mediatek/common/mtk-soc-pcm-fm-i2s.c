...
static int mtk_pcm_fm_i2s_prepare(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == false) {
		...
		if (GetFmI2sInPathEnable() == false) {
			...
			/* Covert from 32000 Hz to 48000 or 44100 Hz */
			if (runtime->rate == 48000)
				SetFmI2sAsrcConfig(true, 48000);
			else
				SetFmI2sAsrcConfig(true, 44100);
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false) {
			SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2,
					    true);
			/* Enable APLL & AFE before Enable I2S */
			EnableAfe(true);
			/* TODO: FM 32bit or 16 bit? */
			Enable_Sync_I2S0_I2S3_With_MCLK(runtime->rate,
