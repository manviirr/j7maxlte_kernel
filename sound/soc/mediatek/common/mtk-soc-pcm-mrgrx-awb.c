...
static uint32 mmrgrx_RecordVol = 0x10000;

static int FM_mrgrx_RecordVol_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s mmrgrx_RecordVol = 0x%x\n", __func__, mmrgrx_RecordVol);
	ucontrol->value.integer.value[0] = mmrgrx_RecordVol;
	return 0;
}

static int FM_mrgrx_RecordVol_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	mmrgrx_RecordVol = ucontrol->value.integer.value[0];
	pr_warn("%s mmrgrx_RecordVol = 0x%x\n", __func__, mmrgrx_RecordVol);
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MRG_I2S_OUT) == true)
		SetHwDigitalGain(mmrgrx_RecordVol, Soc_Aud_Hw_Digital_Gain_HW_DIGITAL_GAIN2);
	return 0;
}

static const struct snd_kcontrol_new Audio_snd_mrgrx_awb_controls[] = {
	SOC_SINGLE_EXT("FM Mrgrx RecordVol", SND_SOC_NOPM, 0, 0x80000, 0,
			FM_mrgrx_RecordVol_Get, FM_mrgrx_RecordVol_Set)
};
#endif
...
static void StopAudioAWBHardware(struct snd_pcm_substream *substream)
{
	...
	SetIntfConnection(Soc_Aud_InterCon_DisConnect,
			Soc_Aud_AFE_IO_Block_MRG_I2S_IN, Soc_Aud_AFE_IO_Block_HW_GAIN2_OUT);
	SetIntfConnection(Soc_Aud_InterCon_DisConnect,
			Soc_Aud_AFE_IO_Block_HW_GAIN2_IN, Soc_Aud_AFE_IO_Block_MEM_AWB);
#else
	SetIntfConnection(Soc_Aud_InterCon_DisConnect,
...
#ifdef CONFIG_MTK_TC10_FEATURE
	snd_soc_add_platform_controls(platform, Audio_snd_mrgrx_awb_controls,
			ARRAY_SIZE(Audio_snd_mrgrx_awb_controls));
#endif
	return 0;
}
