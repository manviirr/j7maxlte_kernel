...
#ifndef MTK_AUDIO_EXT_MCLK_SUPPORT
...
static int mtk_capture_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == false) {
		...
		SetIntfConnection(Soc_Aud_InterCon_Connection, Soc_Aud_AFE_IO_Block_I2S2_ADC,
		...
		EnableAfe(true);
#else
		/* Enable I2S3 MCLK as I2S2 needs I2S3's MCLK */
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false) {
			SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2, true);
			EnableAfe(true);	/*Enable APLL & AFE before Enable I2S */

			/* Afe_Set_Reg(AFE_I2S_CON3, u32AudioI2S | 1, AFE_MASK_ALL); */
			Enable_Sync_I2S0_I2S3_With_MCLK(substream->runtime->rate, mI2SWLen, true);
		} else {
			SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2, true);
		}
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN) == false) {
			...
			/* SetI2SAdcEnable(true); */
			Enable_Sync_I2S1_I2S2(substream->runtime->rate, mI2SWLen, true);
		} else {
			SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN, true);
		}
#endif
...
static int mtk_capture_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == true) {
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN) == false)
			Disable_Sync_I2S1_I2S2();

		/* stop I2S3 MCLK */
		SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2, false);
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false)
			Disable_Sync_I2S0_I2S3_With_MCLK();
#endif
		/* here to turn off digital part */
		SetIntfConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_AFE_IO_Block_I2S2_ADC,
				Soc_Aud_AFE_IO_Block_MEM_VUL);
