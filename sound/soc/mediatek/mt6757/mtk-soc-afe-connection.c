...
bool SetConnsysToHwGain2Out(uint32 ConnectionState)
{
	SetConnectionState(ConnectionState, Soc_Aud_InterConnectionInput_I25,
...
bool SetHwGain2InToAwb(uint32 ConnectionState)
{
...
bool SetMrgI2sInToHwGain2Out(uint32 ConnectionState)
{
	SetConnectionState(ConnectionState, Soc_Aud_InterConnectionInput_I15,
			Soc_Aud_InterConnectionOutput_O15);
	SetConnectionState(ConnectionState, Soc_Aud_InterConnectionInput_I16,
...
#ifdef CONFIG_MTK_TC10_FEATURE
	{Soc_Aud_AFE_IO_Block_I2S_CONNSYS, Soc_Aud_AFE_IO_Block_HW_GAIN2_OUT, SetConnsysToHwGain2Out},
	{Soc_Aud_AFE_IO_Block_HW_GAIN2_IN, Soc_Aud_AFE_IO_Block_MEM_AWB, SetHwGain2InToAwb},
	{Soc_Aud_AFE_IO_Block_MRG_I2S_IN, Soc_Aud_AFE_IO_Block_HW_GAIN2_OUT, SetMrgI2sInToHwGain2Out}
#endif
};
...
{
	int connectReg = 0;
        pr_info("[%s] ConState[%d] Input[%d] -> Output[%d]\n",
		__func__, ConnectionState, Input, Output);

	switch (ConnectionState) {
	case Soc_Aud_InterCon_DisConnect:
	{
		pr_info("[%s] [Soc_Aud_InterCon_DisConnect] ConState[%d]\n",
	...
	{
		pr_info("[%s] [Soc_Aud_InterCon_Connection] ConState[%d]\n",
			__func__, ConnectionState);
		connectReg = mConnectionReg[Output];
		if (CheckBitsandReg(connectReg, Input)) {
	...
	case Soc_Aud_InterCon_ConnectionShift:
	{
		...
		pr_info("[%s] [Soc_Aud_InterCon_ConnectionShift] ConState[%d]\n",
			__func__, ConnectionState);
	...
	default:
		pr_err("[%s] no this state ConnectionState = %d\n",
			__func__, ConnectionState);
		break;
	}
