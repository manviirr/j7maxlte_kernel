...
void SetckSel(uint32 I2snum, uint32 SampleRate)
{
	...
	case Soc_Aud_I2S3:
		...
		ApllSource = 1; /* Fix use APLL2 for I2S3 MCLK */
#endif
...
{
	pr_info("%s: mI2SAPLLDivSelect = %d, i2snum = %d\n",
...
void EnableApll1(bool bEnable)
{
	...
	if (bEnable) {
		if (Aud_APLL_DIV_APLL1_cntr == 0) {
			...
			/* 
			 * set appl1_ck_div0 = 7,
			 * f_faud_engen1_ck = clock source / 8,
...
void EnableApll2(bool bEnable)
{
	...
	if (bEnable) {
		if (Aud_APLL_DIV_APLL2_cntr == 0) {
			...
			/*
			 * set appl2_ck_div0 = 7,
			 * f_faud_engen2_ck = clock source / 8,
			 * 196.608 / 8 = 24.576MHz
			 */
...
uint32 SetCLkMclk(uint32 I2snum, uint32 SampleRate)
{
	...
	case Soc_Aud_I2S3:
		...
		I2s_ck_div = (I2S_APll / MCLKFS / SampleRate) - 1;
#else
		I2s_ck_div = 7; /* Fix MCLK at 196608000/8 = 24.576M */
#endif
...
void SetCLkBclk(uint32 MckDiv, uint32 SampleRate, uint32 Channels, uint32 Wlength)
{
	...
	pr_debug("%s: MckDiv = %dv Rate = %d Channels = %d Wlength = %d\n",
	...
	pr_warn("%s: I2S_APll = %d, I2S_Bclk = %d, I2s_Bck_div = %d\n",
